...
def rasterize_glyph(
    ...
    for poly in subpaths:
        new_poly = [
            ((p[0] - min_x) * scale * ss, (p[1] - min_y) * scale * ss) for p in poly
        ]
    ...
    sorted_paths = sorted(transformed, key=lambda p: abs(polygon_area(p)), reverse=True)
...
def create_stratified_splits(
    ...
    for label, items in by_label.items():
        ...
        # Ensure at least 1 in train if any items exist
        if n_test < 0:
            n_test = 0
            n_val = n - n_train
